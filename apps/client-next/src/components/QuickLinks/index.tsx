...
import { MdBolt as BoltIcon, MdWhatshot as WhatshotIcon, MdConstruction as ConstructionIcon, MdArticle as ArticleIcon, MdCompareArrows as CompareIcon, MdCalculate as CalculateIcon } from 'react-icons/md';
...
const links = [
  ...
  { icon: <CompareIcon size={20} />, labelKey: 'compare.title', href: '/heroes/compare' },
];

export default function QuickLinks() {
  const { t } = useTranslation();
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(3, 1fr)', sm: 'repeat(5, 1fr)' }, gap: { xs: 1, sm: 2 }, mb: 4, px: { xs: 0.5, sm: 0 } }}>
      {links.map(l => (
        <Box
          key={l.href}
          component={Link}
